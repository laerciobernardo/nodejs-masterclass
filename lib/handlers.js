...
handlers._orders.post = function (data, callback) {
   ...
   handlers._tokens.verifyToken(token, email, function (isValid) {
      ...
      _data.read('carts', email, function (err, cartData) {
         ...
         helpers.makeRequest(baseUrl, path, auth, payload, function(stripeResp){
            // console.log('RESPONSE FROM STRIPE');
            console.log(stripeResp);
            let message = 'A new order was registered successfully! U$ ' + amount / 100 + ' I hope that you enjoy!'
            if(stripeResp.paid){
               // helpers.notifyByEmail('New order from pizza', message, function(respEmail){
               //    console.log("RESPONSE FROM MAIL GUN ON HANDLER", respEmail)
                  return callback(200, respEmail);
               })
            }
         });
      })
