...
handlers._checks.post = function(data, callback){
   ...
   let timeoutSecondes = typeof(data.payload.timeoutSecondes) === 'number' && (data.payload.timeoutSecondes % 1 === 0 && data.payload.timeoutSecondes >= 1 && data.payload.timeoutSecondes <= 5) ? data.payload.timeoutSecondes : false;

   if(!protocol || !url || method || sucessCodes || timeoutSeconds) return callback(400, {'Error': 'Missing required fields or are invalids'});
   ...
   _data.read('tokens', token, function(err, tokenData){
      ...
      _data.read('users', userPhone, function(err, userData){
         ...
         const userChecks = typeof(userData.checks) === 'object' && usrData.checks instanceof Array ? userData.checks : [];
         //Veify that the user has more than the number of max-checks-per-user
         if(userChecks.length > config.maxChecks) return callback(400, {'Error': 'The user already has the maximum number of checks ('+config.maxChecks+')'})
